...
export default {
  view: ({ attrs: { unit, index, updateUnit, removeUnit } }) => {
    ...
    return m('.unit', [
      ...
      m('.unit-items', [
        m(
          'div.unit-items-title',
          `Weapons (${weaponsCost}${pointsCostText(weaponsCost)})`,
        ),
      ...
      m('.unit-items', [
        m(
          'div.unit-items-title',
          `Special rules (${rulesCost}${pointsCostText(rulesCost)})`,
        ),
        ...
        m(
          'select',
          {
            oninput: (e) =>
              updateUnit(index, {
                ...unit,
                weapons: [...unit.weapons, weapons[e.target.value]],
              }),
          },
          [
            Object.keys(weapons).map((weapon) =>
              m('option', { value: weapon }, weapons[weapon].name),
            ),
          ],
        ),
      ]),
        ...
        m(
          'select',
          {
            oninput: (e) =>
              updateUnit(index, {
                ...unit,
                rules: [...unit.rules, specialRules[e.target.value]],
              }),
          },
          [
            Object.keys(specialRules).map((rule) =>
              m('option', { value: rule }, specialRules[rule].name),
            ),
          ],
        ),
      ]),
