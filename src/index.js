...
const calculateTotalCost = () => units.reduce((costOfUnits, unit) => {
  return costOfUnits + unit.weapons.reduce(itemsCostReducer, 0) + unit.rules.reduce(itemsCostReducer, 0)
}, 0)
...
const App = {
  view: () => {
    ...
    return m('main', [
      ...
      m('.army-summary', [
        m('span.unit-count', `${units.length} ${units.length === 1 ? 'unit' : 'units'}`),
        m('span.total-cost', `${totalCost}${pointsCostText(totalCost)}`),
      ]),
      units.length === 0
      ? m('.empty-unit-list', 'No units! Add a unit to get started.')
      : m('ul.unit-list', units.map((unit, index) => m('li', m(Unit, { unit, index, updateUnit, removeUnit })))),
      m('button.button.button--full-width', { type: 'button', onclick: () => units.push({ ...DEFAULT_UNIT }) }, 'Add unit')
    ])
  }
}
